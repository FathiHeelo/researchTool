...
export function GroundTruthWarnings({ projectId, runId }: { projectId: string; runId: number }) {
  ...
  return <section className="space-y-2"><button disabled={busy} onClick={() => void load()}>Ground Truth Warnings</button>{busy && <p>Loading warnings…</p>}{error && <p role="alert">{error}</p>}{items && <>
    ...
    <ul>{items.filter(item => (!type || item.warning_type === type) && (!severity || item.severity === severity) && `${item.case_id} ${item.message}`.toLowerCase().includes(search.toLowerCase())).map((item, index) => <li key={index} className="border-t p-2 text-sm">{item.case_id} · {item.warning_type} · <span className="badge" data-severity={item.severity}>{item.severity}</span><p>{item.message}</p><pre className="whitespace-pre-wrap break-words">{item.expected_rule}</pre><details><summary>Case context and related model results</summary><p>{item.requirement}</p><ul>{items.filter(other => other.case_id === item.case_id).map((other, i) => <li key={i}>Result {other.result_id}: {other.model} — {other.warning_type}: {other.message}</li>)}</ul></details></li>)}</ul>
